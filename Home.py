...
def home():
    with st.sidebar:
        ...
        st.session_state["esp_base_url"] = st.text_input(
            ...
            help=(
                "The IP address your ESP32/Pi prints over serial when it connects to WiFi. "
                "Leave blank to run every page in demo mode with sample data."
            ),
        )
        if connected:
    ...
    st.markdown(
        "Edge-AI monitoring for early crop disease detection, environmental sensing, "
        "and climate-resilience alerts — all in one place."
    )
